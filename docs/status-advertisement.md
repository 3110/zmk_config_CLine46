...
`0`〜`8` は `hwinfo_get_reset_cause()`、`9` / `10` は watchdog の記録から取ります。
nRF52 は**電源投入では RESETREAS が 0 のまま**で、UF2 ブートローダが消してしまう
こともあるため、hwinfo が「不明」のときは watchdog に残っている一番新しい記録
（フリーズ・フォールト・記録済みのリセット原因）で補います。それも無ければ `0` です。
